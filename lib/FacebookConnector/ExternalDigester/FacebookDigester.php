...
class FacebookDigester extends DigesterInterface
{
        ...
        if (count($output) === 0) {
            ...
            $mediaAttribute = 'IMAGE_WHATSAPP';
        		...
        		if (isset($message->attributes->$mediaAttribute) && !empty($message->attributes->$mediaAttribute)) {
        			$mediaElements = explode(',',$message->attributes->$mediaAttribute); //TODO explode when multiple files could be configured
        		} else {
        			$mediaElements = [];
        		}
            ...
            if(count($mediaElements) > 0){
              ...
              foreach($mediaElements as $mediaUrl){

                $fileInfo = pathinfo($mediaUrl);

                //FIXME CHANGE FOR attachableFormats
          			$mimeType = $this->mime2ext($fileInfo['extension']);
    ...
    protected function mime2ext($extension) {
          $mime_map = [
              'video/3gpp2'                                                               => '3g2',
              'video/3gp'                                                                 => '3gp',
              'video/3gpp'                                                                => '3gp',
